...
if ($_SERVER["REQUEST_METHOD"] == "POST" ) {
    ...
    try {
        // Define SQL query to insert data into 'photo' table
        $query = "INSERT INTO employee_records (name, position, address, phone_number, age, email, photo) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        // Bind the photo parameter
        $stmt->bindParam(7, $file_name);
       
        // Execute the statement
        if ($stmt->execute() && ($file_name == '' || move_uploaded_file($tempname, $folder))) {
            ...
            echo '<script>alert("Error adding new record: ' . $stmt->errorInfo()[2] . '"); window.location.href = "employee_records.php";</script>';
            exit; // Exit to prevent further execution
        }
