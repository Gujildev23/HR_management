...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['addTask'])) {
        $newTask = [
            "employee_id" => "PC-T" . rand(1000, 9999),
            "task" => $_POST['task'],
            "owner" => $_POST['owner'],
            "status" => $_POST['status'],
        ...
        $stmt = $conn->prepare("INSERT INTO $tasksTable (employee_id, task, owner, status, start_date, due_date, completion, priority, duration) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param(
            "ssssssisi",
            $newTask['employee_id'],
            $newTask['task'],
    ...
    if (isset($_POST['editTask'])) {
        ...
        $stmt = $conn->prepare("UPDATE $tasksTable SET task = ?, owner = ?, status = ?, start_date = ?, due_date = ?, completion = ?, priority = ?, duration = ? WHERE employee_id = ?");
    ...
    if (isset($_POST['deleteTask'])) {
        ...
        if (!empty($selectedIds)) {
            ...
            $stmt = $conn->prepare("DELETE FROM $tasksTable WHERE employee_id IN ($in)");
...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    foreach ($tasks as $task) {
        echo "{$task['employee_id']}\t{$task['task']}\t{$task['owner']}\t{$task['status']}\t{$task['start_date']}\t{$task['due_date']}\t{$task['completion']}\t{$task['priority']}\t{$task['duration']}\n";
    }
    exit();
}

// Initialize sort direction and sort field
$sort = isset($_GET['sort']) ? $_GET['sort'] : 'employee_id'; // Default sort field
...
// Append sorting
if (in_array($sort, ['start_date', 'due_date', 'employee_id', 'task', 'owner', 'status', 'completion', 'duration'])) {
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <input type="checkbox" id="chkbx" name="task_checkbox[]" form="deleteForm" value="<?php echo $task['employee_id']; ?>" onclick="updateSelectedCount(this)">
